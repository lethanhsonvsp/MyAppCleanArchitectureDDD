...
namespace MyApp.Infrastructure.Industrial.Modbus;
...
public class ModbusRtuClient : IDisposable
{
    ...
    public bool IsConnected => _port?.IsOpen == true && !_faulted;
    ...
    private byte[] TxRx(byte[] req, int respLen)
    {
        EnsureConnected();
        if (_port == null || !_port.IsOpen || _faulted)
            throw new InvalidOperationException("Modbus port not available");
        ...
        try
        {
            ...
            // Verify CRC
            if (got < 3) throw new InvalidOperationException("Response too short");
            ushort rxCrc = (ushort)(buf[got - 2] | buf[got - 1] << 8);
            ushort calc = Crc16(buf, got - 2);
            if (rxCrc != calc) throw new InvalidOperationException("CRC mismatch");
    ...
    public ushort[] ReadHoldingRegisters(byte slave, ushort startAddr, ushort quantity)
    {
        ...
        if (resp[0] != slave || resp[1] != 0x03)
            throw new InvalidOperationException("Invalid response function");
        if (resp[2] != byteCount)
            throw new InvalidOperationException("Unexpected byte count");
